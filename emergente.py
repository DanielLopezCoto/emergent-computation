...
class Automata:
	...
	def __init__(self,numeroReglas=100,initSize=149,tiempo=None,initVector=None,initReglas=None):
	...
	def fitnessFun(self,confVectores):
		...
		for i in xrange(numCIs):
			...
			sumaini = float(sum(inicial))/numElementos # Nos dice si la configuracion inicial tiene mas ceros o unos
			...
			if sumaini < 0.5 and sumafin == 0 or sumaini > 0.5 and sumafin == 1:				  # Si el estado inicial tiene mas 0 que unos, cambia fit por 1-fit
				fit.append(1)
			else:
				fit.append(0)
	...
	def algoritmoGenetico(self,tiempo):
		...
		contador.append(self.bondad.index(maxima))
		print contador
...
class AutomImagen:
	...
	def dibujAutom(self):
		...
		for i in xrange(self.nCI):
			c = (regla,i)   # tupla que dara el numero de la regla y de la CI para guardar la imagen.
			matrizEstados = self.m_r[r][i]
